...
def staff_home(request):
    ...
    leave_count = LeaveReportStaff.objects.filter(staff_id__in=staff.id,leave_status=1).count()
    ...
    return render(request, "staff_template/staff_home_template.html",{"students_count":students_count,
                           "attendance_count":attendance_count,"leave_count":leave_count,"subject_count":subject_count,
                      "subject_list":subject_list,"attendance_list":attendance_list,"student_list":student_list,
...
def get_students(request):
    ...
    students= Students.objects.filter(course_id=subject.course_id,session_year=session_model)
...
def save_attendance_data(request):
    ...
    # print(json_students[0]['id'])

    try:
        subject_model=Subjects.objects.get(id=subject_id)
        session_model=Subjects.objects.get(id=session_year_id)
...
def save_updateattendance_data(request):
    student_ids = request.POST.get("student_ids")
    attendance_date = request.POST.get("attendance_date")
    attendance= Attendance.get(id=attendance_date)
...
def staff_feedback(request):
    staff_obj = Staffs.objects.get(admin=request.user.id)
    feedback_data = LeaveReportStaff.objects.filter(staff_id=staff_obj)
    return render(request, "staff_template/staff_feedback_leave.html",{"feedback_data":feedback_data})
